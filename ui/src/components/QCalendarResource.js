  ...
  methods: {
    __renderHeadIntervals(h) {
      return h(
        ...
        [
          this.intervals.map((intervals) =>
            intervals.map((interval, index) =>
              this.__renderHeadInterval(h, interval, index)
            )
          ),
        ]
      );
        ...
        : h(
            "div",
            {
              ...
              on: this.getDefaultMouseEventHandlers(
                ...
                (event, eventName) => {
                  return {
                    scope: { timestamp: interval, index, label },
                    event,
                  };
                }
              ),
      ...
      return h(
        "div",
        {
          staticClass:
            ...
            (this.sticky === true ? " q-calendar__sticky" : ""),
        },
        [this.__renderHeadResource(h), this.__renderHeadIntervals(h)]
      );
    },
    ...
    __renderResourceRow(h, resource, idx, indentLevel = 0) {
      ...
      if (resource.expanded === true) {
        return [
          resourceRow,
          ...this.__renderResources(h, resource.children, indentLevel + 1),
        ];
      }
    ...
    __renderResourceLabel(h, resource, idx, indentLevel = 0) {
      ...
      const height =
        resource.height !== void 0
          ...
          : convertToUnit(this.parsedResourceHeight);
      ...
      return h(
        "div",
        {
          ...
          on: this.getDefaultMouseEventHandlers(
            ...
            (event, eventName) => {
              return {
                scope: { resource, index: idx, intervals: this.intervals },
                event,
              };
            }
          ),
        ...
        [
          slot
            ? slot(scope)
            : this.__renderResourceText(h, resource, idx, indentLevel),
        ]
      );
    },
    ...
    __renderResourceText(h, resource, idx, indentLevel = 0) {
      ...
      if (label === undefined) {
        ...
        console.warn(
          'QCalendarResource: resource object requires "resource-key" property to contain resource object key'
        );
      }
        ...
        [
          this.intervals.map((intervals) =>
            intervals.map((interval) =>
              this.__renderResourceInterval(h, resource, interval, idx)
            )
          ),
          slot &&
            slot({ resource, intervals, timeStartPosX, timeDurationWidth }),
        ]
      );
    },
      ...
      return h(
        "div",
        {
          ...
          on: this.getDefaultMouseEventHandlers(
            ":time2",
            (event, eventName) => {
              const scope = this.getScopeForSlotX(
                this.getTimestampAtEventX(
                  event,
                  interval,
                  this.timeClicksClamped
                )
              );
